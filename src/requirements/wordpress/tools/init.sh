...
# if [ ! -f "/var/www/wordpress/wp-config.php" ]; then
	...
	# if ! mysqladmin -h $MARIADB_DATABASE_HOST -u $MARIADB_ADMIN_USER --password=$MARIADB_ADMIN_PASSWORD --wait=42 ping > /dev/null; then
	# 	echo -e "${RED} Mariadb is not reachable\n ${RESET}"
	# 	exit 1
	# fi
	wp core download --path=/var/www/wordpress --allow-root
	wp config create --dbname=$MARIADB_DATABASE --dbuser=$MARIADB_USER --dbpass=$MARIADB_PASSWORD --dbhost=$MARIADB_DATABASE_HOST --dbcharset="utf8" --allow-root
